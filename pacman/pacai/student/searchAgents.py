...
class CornersProblem(SearchProblem):
    ...
    def startingState(self):
        return ((self.startingPosition), tuple([0, 0, 0, 0]))
        ...
        for action in Directions.CARDINAL:
            ...
            if (not hitsWall):
                ...
                if (nextx, nexty) in self.corners:
                    ...
                    StateList = list(state[1])
                    StateList[index] = 1

                    # Append Children in corners
                    successors.append((((nextx, nexty), tuple(StateList)), action, 1))
 
                else:
                    # append children not in corners
...
def cornersHeuristic(state, problem):
    ...
    # if still looking
        ...
        # Each corner coordinate
        for corner in corners:
            Distances = []  # when init here, 200 nodes smaller, following one at a time?
            mDistance = distance.manhattan(currentCoordinates, corner)
            Distances.append(mDistance)
           
        # return min of places
...
def foodHeuristic(state, problem):
    ...
    # *** Your Code Here ***
    groceries = foodGrid.asList()
    Distances = [0]
    # Each corner coordinate
    for food in groceries:
        # calc maze distance, append
        mazeDistance = distance.maze(position, food, problem.startingGameState)
        Distances.append(mazeDistance)
            
    # return max of places
    return max(Distances)
